...
function subtractTwo() {
    ...
    if (cartQuantity <= 1) {
        ...
        function computerPlay() {
            computerChoice = Math.random() >= 0.5 ? 'tails' : 'heads';
            console.log(computerChoice);
        }

        function result() {
            if (playerChoice === computerChoice) {
                resultDisplay = document.getElementById("result").innerHTML = 'You win!';
            } else {
                resultDisplay = document.getElementById("result").innerHTML = 'You lose!';
            }
        } console.log('Cart Quantity:', cartQuantity);
    } else {
        console.log('not a valid input');
